...
faq:
  ...
  - q: Can removing hidden Unicode bypass AI text detectors?
    a: No tool guarantees bypassing AI detection. Hidden Unicode characters are ordinary character data rather than statistical watermarks, and removing them does not affect detector output in any way NoWatermark has measured. That is a statement about what we have tested, not a promise about any particular detector.
---
...
You can inspect text directly in your browser using the [hidden character checker](/claude-watermark-checker). All scanning runs entirely client-side, meaning your pasted text is never transmitted to an external server or stored in a remote database.
...
1. **Scan the input**: Paste your text into the [hidden character checker](/claude-watermark-checker). The detector inspects every code point against the registry, flagging isolated zero-width characters, bidi overrides, and tag characters while preserving valid emoji sequences.
2. **Remove unwanted characters**: Use the [hidden character remover](/claude-watermark-remover) to strip non-functional invisible characters from the string.

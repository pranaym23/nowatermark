...
3. **A web platform re-encoded the file.** Many services re-encode images on upload, and re-encoding commonly discards metadata blocks. NoWatermark has not tested individual platforms, so treat this as a mechanism to be aware of rather than a statement about any particular service — the reliable move is to check the specific file you have.
